...
<form:form modelAttribute="trust">
    ...
    <table >
        <tr>
            <td>
                <a href="<c:url value="/institute/addTrust.do"/>"><fmt:message key="common.add"/></a>
            </td>
        </tr>
    ...
    <div id="advance" style="display:none">
        <table>
            ...
            <tr>
                <td><fmt:message key="institute.trust-name"/></td>
                <td><fmt:message key="common.city"/></td>
            </tr>
            <tr>
                <td><form:input path="trustName" size="30"/></td>
    ...
    <table border="1">
        <thead>
            <tr>
                ...
                <th><fmt:message key="institute.trust-name"/></th>
                <th><fmt:message key="common.city"/></th>
                <th><fmt:message key="common.phoneNumber1"/></th>
            </tr>
        </thead>
        <tbody>
            <c:choose>
                ...
                <c:otherwise>
                    <c:forEach var="trust" items="${selections}" varStatus="status">
                        <tr>
                            <td><intput type="checkbox" name="cb" id="${trust.trustId}"/></td>
                            <td><a href="<c:url value="/institute/updateTrust.do?trustId=${trust.trustId}"/>">${trust.trustId}</a></td>
                            <td>${trust.trustName}</td>
                            <td>${trust.address.city}</td>
                            <td>${trust.address.phoneNumber1}</td>
                        </tr>
                    </c:forEach>
                </c:otherwise>
            </c:choose>
